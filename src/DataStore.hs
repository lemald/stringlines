...
insertTripInfo con ts =
  executeMany
  con
  "INSERT INTO location (trip_id, route_id, direction_id, latitude, longitude, timestamp) SELECT ?,?,?,?,?,? WHERE NOT EXISTS (SELECT 1 FROM location WHERE trip_id = ? AND timestamp = ?)"
