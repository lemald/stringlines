...
testRowInsert con = do
  ...
  tripInfoEntries <- query_
    con
    "SELECT CAST(trip_id AS TEXT), CAST(route_id AS TEXT), direction_id, CAST(vehicle_label AS TEXT), CAST(vehicle_id AS TEXT), latitude, longitude, progress, timestamp FROM location"
    :: IO([TripInfo])
